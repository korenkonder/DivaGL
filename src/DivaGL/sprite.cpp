            ...
            for (uint32_t j = SPR_PRIO_MAX; j; j--, reqlist++)
                for (auto& k : *reqlist)
                    draw_sprite(k, font, mat, x_min, y_min, x_max, y_max,
                        sprite_manager_render_data->draw_param_buffer,
                        sprite_manager_render_data->vertex_buffer);

            static const GLsizei buffer_size = sizeof(sprite_draw_vertex);

            sprite_manager_render_data->Update();

            rctx->sprite_scene_ubo.Bind(0);

            gl_state_active_bind_texture_2d(7, tex->tex);

            gl_state_bind_vertex_array(sprite_manager_render_data->vao);
            for (sprite_draw_param& j : sprite_manager_render_data->draw_param_buffer) {
                if (j.blend) {
